...
# `bin/agw` is not guaranteed to be on PATH under Codex the way it is for Claude
# plugins, so spell out an unambiguous fallback invocation.
_AGW = os.path.join(PLUGIN_ROOT, "bin", "agw")
_AGW_PY = os.path.join(PLUGIN_ROOT, "scripts", "agw", "agw.py")

CONTEXT = f"""agentic-guardrails is active in this session. Run the CLI as `agw \
<cmd>` if it is on PATH, otherwise `{_AGW}` or `python3 {_AGW_PY}`. File-safety rules:
- Deletion is disabled - both shell `rm` and `apply_patch` "Delete File" blocks. \
Use `agw archive <path>` (reversible via `agw restore <path>`); `agw undo` reverts \
the last operation.
